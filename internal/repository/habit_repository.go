...
func (r *habitRepository) Create(ctx context.Context, h *models.Habit) (int64, error) {
	res, err := r.db.ExecContext(ctx, `INSERT INTO habits (user_id, name, description, created_at) VALUES (?, ?, ?, ?)`,
		h.UserID, h.Name, h.Description, time.Now().UTC())
	if err != nil { return 0, err }
	return res.LastInsertId()
}

func (r *habitRepository) GetByID(ctx context.Context, id int64) (*models.Habit, error) {
	row := r.db.QueryRowContext(ctx, `SELECT id, user_id, name, description, created_at FROM habits WHERE id = ?`, id)
	h := &models.Habit{}
...
func (r *habitRepository) ListByUser(ctx context.Context, userID int64) ([]*models.Habit, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id, user_id, name, description, created_at FROM habits WHERE user_id = ? ORDER BY id DESC`, userID)
	if err != nil { return nil, err }
	defer rows.Close()
	var out []*models.Habit
	for rows.Next() {
		h := &models.Habit{}
		if err := rows.Scan(&h.ID, &h.UserID, &h.Name, &h.Description, &h.CreatedAt); err != nil { return nil, err }
		out = append(out, h)
	}
	return out, rows.Err()
}
...
func (r *habitRepository) Update(ctx context.Context, h *models.Habit) error {
	_, err := r.db.ExecContext(ctx, `UPDATE habits SET name = ?, description = ? WHERE id = ? AND user_id = ?`, h.Name, h.Description, h.ID, h.UserID)
	return err
}
