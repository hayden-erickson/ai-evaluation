...
func (r *habitRepository) Create(habit *models.Habit) error {
	query := `
		INSERT INTO habits (user_id, name, description, duration, created_at)
		VALUES (?, ?, ?, ?, ?)
	`

	result, err := r.db.Exec(query, habit.UserID, habit.Name, habit.Description, habit.Duration, habit.CreatedAt)
	if err != nil {
