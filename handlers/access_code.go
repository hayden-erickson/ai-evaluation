...
// UserAccessCodeEdit records user access code edit activity
...
func AccessCodeEditHandler(w http.ResponseWriter, r *http.Request) {
	claims, err := contextutil.GetClaimsFromContext(r.Context())
	if err != nil {
	...
	bank, ok := contextutil.BankFromContext(r.Context())
	if !ok {
	...
	for _, unitID := range input.UnitIDs {
		...
		if unitID != 0 {
			...
			if unit.RentalState == constants.LockStateOverlock || unit.RentalState == constants.LockStateGatelock ||
